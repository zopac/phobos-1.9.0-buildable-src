...
public class RenderUtil
    ...
    public static void updateModelViewProjectionMatrix() {
        ...
        final ScaledResolution res = new ScaledResolution(Minecraft.instance);
        GLUProjection.getInstance().updateMatrices(RenderUtil.viewport, RenderUtil.modelView, RenderUtil.projection, res.getScaledWidth() / (float)Minecraft.instance.displayWidth, res.getScaledHeight() / (float)Minecraft.instance.displayHeight);
    }
    ...
    public static void drawGradientBlockOutline(BlockPos pos, Color startColor, Color endColor, float linewidth) {
        ...
        RenderUtil.drawGradientBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtil.mc.world, pos).grow(0.002f).offset(-interp.x, -interp.y, -interp.z), startColor, endColor, linewidth);
    }
    ...
    public static void drawBoxESP(BlockPos pos, Color color, boolean secondC, Color secondColor, float lineWidth, boolean outline, boolean box, int boxAlpha, boolean air, double height) {
        if (box) {
            RenderUtil.drawBox(pos, new Color(color.getRed(), color.getGreen(), color.getBlue(), boxAlpha), height);
        }
        if (outline) {
            RenderUtil.drawBlockOutline(pos, secondC ? secondColor : color, lineWidth, air, height);
        }
    }
    ...
    public static boolean isInViewFrustrum(AxisAlignedBB bb) {
        Entity current = Minecraft.instance.getRenderViewEntity();
